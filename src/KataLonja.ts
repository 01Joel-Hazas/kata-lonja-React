...
export function enviarDatosReact(vieira: number, pulpo: number, centollo: number) {
  ...
  function calcularGananciasProductos(producto: IPescado, ciudad: ICiudad) {
    ...
    let precioProd: number = producto.nombrePescado === "vieiras" ? ciudad.precioPescados.vieiras:(producto.nombrePescado==="pulpo"?ciudad.precioPescados.pulpo:ciudad.precioPescados.centollos)
  ...
  function calcularGananciasPorCiudades(productos: IPescado[], ciudad: ICiudad) {

    let gananciasVieira: number = calcularGananciasProductos(pescados[0], ciudad);
    let gananciasPulpo: number = calcularGananciasProductos(pescados[1], ciudad);
    let gananciasCentollo: number = calcularGananciasProductos(pescados[2], ciudad);
  ...
  function calcularCiudadConMejorBeneficio(productos: IPescado[], ciudades: ICiudad[]) {
